...
def process():
    ...
    yield from configure_node(nodes[2], ConfigUp.r, 0)
    yield from configure_node(nodes[3], ConfigUp.l, 0)
    yield from configure_node(nodes[4], ConfigUp.sum_l_r_f, 0)
    yield from configure_node(nodes[5], ConfigForward.sum_l_r, 0)
    yield from configure_node(nodes[6], ConfigUp.r, 0)
    yield Tick()

    yield from configure_node(nodes[1], ConfigUp.sum_l_r, 0)
    yield Tick()
